...
import { getClassicAudioInput, getDSK, getFairlightAudioInput, getFairlightAudioMasterChannel, getFairlightAudioMonitorChannel, getMixEffect, getSuperSourceBox, getUSK, } from '../state.js';
...
function getSourcePresetName(instance, state, id) {
...
export function updateChangedVariables(instance, state, changes) {
    const newValues = {};
    for (const meIndex of changes.meProgram)
        updateMEProgramVariable(instance, state, meIndex, newValues);
    for (const meIndex of changes.mePreview)
        updateMEPreviewVariable(instance, state, meIndex, newValues);
    for (const meIndex of changes.transitionPosition)
        updateMETransitionPositionVariable(state, meIndex, newValues);
    for (const auxIndex of changes.auxes)
        updateAuxVariable(instance, state, auxIndex, newValues);
    for (const dsk of changes.dsk)
        updateDSKVariable(instance, state, dsk, newValues);
    for (const [meIndex, usk] of changes.usk)
        updateUSKVariable(instance, state, meIndex, usk, newValues);
    for (const macro of changes.macros)
        updateMacroVariable(state, macro, newValues);
    for (const ssrc of changes.ssrc)
        updateSuperSourceVariables(instance, state, ssrc, newValues);
    for (const mp of changes.mediaPlayer)
        updateMediaPlayerVariables(state, mp, newValues);
    if (changes.recording)
        updateRecordingVariables(state, newValues);
    if (changes.streaming)
        updateStreamingVariables(state, newValues);
    for (const fairlightAudioIndex of changes.fairlightAudio)
        updateFairlightAudioVariables(state, fairlightAudioIndex, newValues);
    for (const classicAudioIndex of changes.classicAudio)
        updateClassicAudioVariables(state, classicAudioIndex, newValues);
    if (changes.fairlightAudioMaster)
        updateFairlightAudioMasterVariables(state, newValues);
    if (changes.fairlightAudioMonitor)
        updateFairlightAudioMonitorVariables(state, newValues);
...
function updateMEProgramVariable(instance, state, meIndex, values) {
...
function updateMEPreviewVariable(instance, state, meIndex, values) {
...
function updateMETransitionPositionVariable(state, meIndex, values) {
...
function updateUSKVariable(instance, state, meIndex, keyIndex, values) {
...
function updateDSKVariable(instance, state, keyIndex, values) {
...
function updateAuxVariable(instance, state, auxIndex, values) {
...
function updateInputVariables(src, values) {
    values[`long_${src.id}`] = src.longName;
    values[`short_${src.id}`] = src.shortName;
}
function updateStreamingVariables(state, values) {
...
function updateRecordingVariables(state, values) {
...
function formatAudioProperty(value, scale = 100) {
    if (value === undefined) {
        return;
    }
    return (value / scale).toString();
}
function formatFairlightAudioMixOption(value) {
...
function formatAudioMixOption(value) {
    let mixOption = undefined;
    switch (value) {
...
function updateFairlightAudioVariables(state, fairlightAudioIndex, values) {
    ...
    if (sources !== undefined && sources[-256]) {
        ...
        values[`audio_input_${fairlightAudioIndex}_left_mixOption`] = formatFairlightAudioMixOption(leftProperties?.mixOption);
    }
    if (sources !== undefined && sources[-255]) {
        ...
        values[`audio_input_${fairlightAudioIndex}_right_mixOption`] = formatFairlightAudioMixOption(rightProperties?.mixOption);
    }
}
function updateClassicAudioVariables(state, classicAudioIndex, values) {
...
function updateFairlightAudioMasterVariables(state, values) {
...
function updateFairlightAudioMonitorVariables(state, values) {
...
function updateSuperSourceVariables(instance, state, i, values) {
    for (let b = 0; b < 4; b++) {
...
function updateMultiviewerWindowInput(instance, state, index, // 1 index
window, // 1 index
values) {
...
export function updateDeviceIpVariable(instance, values) {
    values['device_ip'] = instance.parseIpAndPort()?.ip || '';
}
export function InitVariables(instance, model, state) {
    const variables = [];
    const values = {};
    variables.push({
    ...
    if (instance.config.enableCameraControl) {
        for (const input of model.inputs) {
            if (input.portType !== Enums.InternalPortType.External)
                continue;
