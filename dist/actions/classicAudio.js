...
import { AtemAudioInputPicker, FadeDurationChoice, FaderLevelDeltaChoice } from '../input.js';
export function createClassicAudioActions(atem, model, transitions, state) {
    ...
    return {
        [ActionId.ClassicAudioGain]: {
            ...
            callback: async ({ options }) => {
                ...
                await transitions.run(`audio.${inputId}.gain`, async (value) => {
                    await atem?.setClassicAudioMixerInputProps(inputId, { gain: value });
                }, channel?.gain, options.getPlainNumber('gain'), options.getPlainNumber('fadeDuration'));
            },
            learn: ({ options }) => {
                const audioChannels = state.state.audio?.channels ?? {};
        ...
        [ActionId.ClassicAudioGainDelta]: {
            ...
            callback: async ({ options }) => {
                ...
                if (typeof channel?.gain === 'number') {
                    await transitions.run(`audio.${inputId}.gain`, async (value) => {
                        await atem?.setClassicAudioMixerInputProps(inputId, { gain: value });
                    }, channel.gain, channel.gain + options.getPlainNumber('delta'), options.getPlainNumber('fadeDuration'));
                }
            },
        },
        [ActionId.ClassicAudioMixOption]: {
            ...
            callback: async ({ options }) => {
                const inputId = options.getPlainNumber('input');
                const audioChannels = state.state.audio?.channels ?? {};
                const toggleVal = audioChannels[inputId]?.mixOption === Enums.AudioMixOption.On
        ...
        [ActionId.ClassicAudioMasterGain]: {
            ...
            callback: async ({ options }) => {
                ...
                }, state.state.audio?.master?.gain, options.getPlainNumber('gain'), options.getPlainNumber('fadeDuration'));
            },
            learn: ({ options }) => {
        ...
        [ActionId.ClassicAudioMasterGainDelta]: {
            ...
            callback: async ({ options }) => {
                ...
                if (typeof currentGain === 'number') {
                    await transitions.run(`audio.master.gain`, async (value) => {
                        await atem?.setClassicAudioMixerMasterProps({ gain: value });
                    }, currentGain, currentGain + options.getPlainNumber('delta'), options.getPlainNumber('fadeDuration'));
                }
            },
        },
        [ActionId.ClassicAudioMasterPan]: {
            ...
            callback: async ({ options }) => {
                await transitions.run(`audio.master.pan`, async (value) => {
                    await atem?.setClassicAudioMixerMasterProps({ balance: value });
                }, state.state.audio?.master?.balance, options.getPlainNumber('balance'), options.getPlainNumber('fadeDuration'));
            },
            learn: ({ options }) => {
                const props = state.state.audio?.master;
                if (props) {
        ...
        [ActionId.ClassicAudioMasterPanDelta]: {
            ...
            callback: async ({ options }) => {
                ...
                if (typeof currentBalance === 'number') {
                    await transitions.run(`audio.master.gain`, async (value) => {
                        await atem?.setClassicAudioMixerMasterProps({ balance: value });
                    }, currentBalance, currentBalance + options.getPlainNumber('delta'), options.getPlainNumber('fadeDuration'));
                }
            },
        },
    };
}
//# sourceMappingURL=classicAudio.js.map