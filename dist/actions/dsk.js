...
import { CHOICES_KEYTRANS, CHOICES_ON_OFF_TOGGLE, GetDSKIdChoices } from '../choices.js';
import { AtemDSKPicker, AtemKeyFillSourcePicker, AtemKeyCutSourcePicker, AtemRatePicker, AtemDSKMaskPropertiesPickers, AtemDSKPreMultipliedKeyPropertiesPickers, } from '../input.js';
import { getDSK } from '../state.js';
export function createDownstreamKeyerActions(atem, model, state) {
    if (!model.DSKs) {
        return {
    ...
    return {
        ...
        [ActionId.DSKSourceVariables]: {
            ...
            callback: async ({ options }) => {
                ...
                if (isNaN(key) || isNaN(fill) || isNaN(cut))
                    return;
