...
export function createDisplayClockActions(atem, model, state) {
            ...
            callback: async ({ options }) => {
                ...
                if (props && Array.isArray(props)) {
                    if (props.includes('enabled'))
                        newProps.enabled = options.getPlainBoolean('enabled');
                    if (props.includes('size'))
                        newProps.size = options.getPlainNumber('size') * 100;
                    if (props.includes('opacity'))
                        newProps.opacity = options.getPlainNumber('opacity') * 100;
                    if (props.includes('x'))
                        newProps.positionX = options.getPlainNumber('x') * 1000;
                    if (props.includes('y'))
                        newProps.positionY = options.getPlainNumber('y') * 1000;
                    if (props.includes('autoHide'))
                        newProps.autoHide = options.getPlainBoolean('autoHide');
                    if (props.includes('clockMode'))
                        newProps.clockMode = options.getPlainNumber('clockMode');
                }
...
//# sourceMappingURL=displayClock.js.map