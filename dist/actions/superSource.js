import { Enums, VideoState } from 'atem-connection';
import { ActionId } from './ActionId.js';
import { AtemSuperSourceArtPropertiesPickers, AtemSuperSourceBoxPicker, AtemSuperSourceBoxSourcePicker, AtemSuperSourceIdPicker, AtemSuperSourcePropertiesPickers, AtemSuperSourcePropertiesPickersForOffset, } from '../input.js';
import { CHOICES_KEYTRANS } from '../choices.js';
import { getSuperSourceBox } from '../state.js';
import { clamp } from '../util.js';
export function createSuperSourceActions(atem, model, state) {
    if (!model.SSrc) {
        return {
    ...
    return {
            ...
            callback: async ({ options }) => {
                ...
                if (props && Array.isArray(props)) {
                    if (props.includes('fill'))
                        newProps.artFillSource = options.getPlainNumber('fill');
                    if (props.includes('key'))
                        newProps.artCutSource = options.getPlainNumber('key');
                    ...
                    if (props.includes('artClip'))
                        newProps.artClip = options.getPlainNumber('artClip') * 10;
                    if (props.includes('artGain'))
                        newProps.artGain = options.getPlainNumber('artGain') * 10;
                    if (props.includes('artInvertKey'))
                        newProps.artInvertKey = options.getPlainBoolean('artInvertKey');
                }
        ...
        [ActionId.SuperSourceBoxSource]: {
            ...
            callback: async ({ options }) => {
                ...
                }, options.getPlainNumber('boxIndex'), options.getRaw('ssrcId') && model.SSrc > 1 ? Number(options.getRaw('ssrcId')) : 0);
            },
            learn: ({ options }) => {
                const ssrcId = options.getRaw('ssrcId') && model.SSrc > 1 ? Number(options.getRaw('ssrcId')) : 0;
        ...
        [ActionId.SuperSourceBoxSourceVaraibles]: {
            ...
            callback: async ({ options }) => {
                const ssrcId = options.getRaw('ssrcId') && model.SSrc > 1 ? Number(await options.getParsedNumber('ssrcId')) - 1 : 0;
                const boxIndex = (await options.getParsedNumber('boxIndex')) - 1;
                const source = await options.getParsedNumber('source');
                if (isNaN(ssrcId) || isNaN(boxIndex) || isNaN(source))
                    return;
                await atem?.setSuperSourceBoxSettings({
                    source: source,
                }, boxIndex, ssrcId);
            ...
            callback: async ({ options }) => {
                ...
                if (props && Array.isArray(props)) {
                    ...
                    if (props.includes('source'))
                        newProps.source = options.getPlainNumber('source');
                    if (props.includes('size'))
                        newProps.size = options.getPlainNumber('size') * 1000;
                    if (props.includes('x'))
                        newProps.x = options.getPlainNumber('x') * 100;
                    if (props.includes('y'))
                        newProps.y = options.getPlainNumber('y') * 100;
                    if (props.includes('cropEnable'))
                        newProps.cropped = options.getPlainBoolean('cropEnable');
                    if (props.includes('cropTop'))
                        newProps.cropTop = options.getPlainNumber('cropTop') * 1000;
                    if (props.includes('cropBottom'))
                        newProps.cropBottom = options.getPlainNumber('cropBottom') * 1000;
                    if (props.includes('cropLeft'))
                        newProps.cropLeft = options.getPlainNumber('cropLeft') * 1000;
                    if (props.includes('cropRight'))
                        newProps.cropRight = options.getPlainNumber('cropRight') * 1000;
                }
                if (Object.keys(newProps).length === 0)
                    return;
        ...
        [ActionId.SuperSourceBoxPropertiesDelta]: {
            ...
            callback: async ({ options }) => {
                ...
                if (box && props && Array.isArray(props)) {
                    if (props.includes('size'))
                        newProps.size = clamp(0, 1000, box.size + options.getPlainNumber('size') * 1000);
                    if (props.includes('x'))
                        newProps.x = clamp(-4800, 4800, box.x + options.getPlainNumber('x') * 100);
                    if (props.includes('y'))
                        newProps.y = clamp(-2700, 2700, box.y + options.getPlainNumber('y') * 100);
                    if (props.includes('cropTop'))
