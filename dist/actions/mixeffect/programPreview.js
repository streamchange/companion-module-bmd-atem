...
export function createProgramPreviewActions(atem, model, transitions, state) {
    return {
        [ActionId.Program]: {
        ...
        [ActionId.TBar]: {
            ...
            callback: async ({ options }) => {
                ...
                await transitions.run(`me.${meIndex}.tbar`, async (value) => {
                    await atem?.setTransitionPosition(value, meIndex);
                }, meState?.transitionPosition.handlePosition, position * 100, options.getPlainNumber('fadeDuration'));
            },
        },
    };
}
