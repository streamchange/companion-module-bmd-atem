...
import { AtemKeyCutSourcePicker, AtemKeyFillSourcePicker, AtemMEPicker, AtemUSKMaskPropertiesPickers, AtemUSKPicker, AtemUpstreamKeyerTypePicker, } from '../../input.js';
import { ActionId } from '../ActionId.js';
import { CHOICES_KEYTRANS } from '../../choices.js';
import { getUSK } from '../../state.js';
export function createUpstreamKeyerCommonActions(atem, model, state) {
    if (!model.USKs) {
        return {
    ...
    return {
        [ActionId.USKSource]: {
            ...
            callback: async ({ options }) => {
                await Promise.all([
                    atem?.setUpstreamKeyerFillSource(options.getPlainNumber('fill'), options.getPlainNumber('mixeffect'), options.getPlainNumber('key')),
                    atem?.setUpstreamKeyerCutSource(options.getPlainNumber('cut'), options.getPlainNumber('mixeffect'), options.getPlainNumber('key')),
                ]);
            },
            learn: ({ options }) => {
        ...
        [ActionId.USKType]: {
            ...
            callback: async ({ options }) => {
                ...
                }, options.getPlainNumber('mixeffect'), options.getPlainNumber('key'));
            },
            learn: ({ options }) => {
                const usk = getUSK(state.state, options.getPlainNumber('mixeffect'), options.getPlainNumber('key'));
                if (usk) {
        ...
        [ActionId.USKSourceVariables]: {
            ...
            callback: async ({ options }) => {
                ...
                if (isNaN(mixeffect) || isNaN(key) || isNaN(fill) || isNaN(cut))
                    return;
                await Promise.all([
...
//# sourceMappingURL=upstreamKeyerCommon.js.map