...
import { MODEL_AUTO_DETECT } from './types.js';
...
export function GetParsedModelSpec({ info, inputs, settings, streaming, recording, audio, fairlight, displayClock, }) {
    ...
    return {
        ...
        multiviewerFullGrid: (settings.multiViewers?.[0]?.windows.length || 0) > 10,
        ...
        classicAudio: audio
            ? {
                inputs: compact(Object.entries(audio.channels).map(([id, ch]) => {
                    if (!ch?.portType)
                        return undefined;
                    return {
                        id: Number(id),
        ...
        fairlightAudio: fairlight
            ? {
                ...
                inputs: compact(Object.entries(fairlight.inputs).map(([id, ch]) => {
                    if (!ch?.properties)
                        return undefined;
                    return {
                        id: Number(id),
