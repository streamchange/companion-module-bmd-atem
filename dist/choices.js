...
export const CHOICES_SSRCBOXES = [
    { id: 0, label: 'Box 1' },
    { id: 1, label: 'Box 2' },
    { id: 2, label: 'Box 3' },
    { id: 3, label: 'Box 4' },
];
export const CHOICES_BORDER_BEVEL = [
    { id: 0, label: 'None' },
    { id: 1, label: 'InOut' },
    { id: 2, label: 'In' },
    { id: 3, label: 'Out' },
];
export const CHOICES_ON_OFF_TOGGLE = [
    { id: 'true', label: 'On' },
    { id: 'false', label: 'Off' },
    { id: 'toggle', label: 'Toggle' },
];
export var NextTransBackgroundChoices;
(function (NextTransBackgroundChoices) {
    NextTransBackgroundChoices["NoChange"] = "no-change";
    NextTransBackgroundChoices["Include"] = "include";
    NextTransBackgroundChoices["Omit"] = "omit";
    NextTransBackgroundChoices["Toggle"] = "toggle";
})(NextTransBackgroundChoices || (NextTransBackgroundChoices = {}));
export const CHOICES_NEXTTRANS_BACKGROUND = [
...
export var NextTransKeyChoices;
(function (NextTransKeyChoices) {
    NextTransKeyChoices["NoChange"] = "no-change";
    NextTransKeyChoices["On"] = "on";
    NextTransKeyChoices["Off"] = "off";
    NextTransKeyChoices["Toggle"] = "toggle";
    NextTransKeyChoices["Include"] = "include";
    NextTransKeyChoices["Omit"] = "omit";
})(NextTransKeyChoices || (NextTransKeyChoices = {}));
export const CHOICES_NEXTTRANS_KEY = [
...
export const CHOICES_CLASSIC_AUDIO_MIX_OPTION = [
    {
...
export const CHOICES_FAIRLIGHT_AUDIO_MIX_OPTION = [
    {
...
export function GetTransitionStyleChoices(skipSting) {
    const options = [
...
export function GetUpstreamKeyerTypeChoices() {
    const options = [
...
export function GetMEIdChoices(model) {
    return iterateTimes(model.MEs, (i) => ({
        id: i,
        label: `M/E ${i + 1}`,
    }));
}
export function GetAuxIdChoices(model) {
    return iterateTimes(model.auxes, (i) => ({
        id: i,
        label: `${i + 1}`,
    }));
}
export function GetUSKIdChoices(model) {
    return iterateTimes(model.USKs, (i) => ({
        id: i,
        label: `${i + 1}`,
    }));
}
export function GetDSKIdChoices(model) {
    return iterateTimes(model.DSKs, (i) => ({
        id: i,
        label: `${i + 1}`,
    }));
}
export function GetMultiviewerIdChoices(model) {
    return iterateTimes(model.MVs, (i) => ({
        id: i,
        label: `MV ${i + 1}`,
    }));
}
export function GetSuperSourceIdChoices(model) {
    return iterateTimes(model.SSrc, (i) => ({
        id: i,
        label: `Super Source ${i + 1}`,
    }));
}
export function GetMacroChoices(model, state) {
...
export function GetMediaPlayerChoices(model) {
...
export function GetSourcesListForType(model, state, subset) {
    const getSource = (id, defShort, defLong) => {
    ...
    for (const input of model.inputs) {
        switch (subset) {
            case undefined:
                break;
            case 'me':
                if (input.meAvailability === Enums.MeAvailability.None)
                    continue;
                break;
            case 'aux':
                if ((input.sourceAvailability & Enums.SourceAvailability.Auxiliary) === 0)
                    continue;
                break;
            case 'mv':
                if ((input.sourceAvailability & Enums.SourceAvailability.Multiviewer) === 0)
                    continue;
                break;
            case 'key':
                if ((input.sourceAvailability & Enums.SourceAvailability.KeySource) === 0)
                    continue;
                break;
            case 'ssrc-box':
                if ((input.sourceAvailability & Enums.SourceAvailability.SuperSourceBox) === 0)
                    continue;
                break;
            case 'ssrc-art':
                if ((input.sourceAvailability & Enums.SourceAvailability.SuperSourceArt) === 0)
                    continue;
                break;
            case 'tally':
                // if (input.portType === Enums.InternalPortType.Auxiliary) break // TODO: Future
                if (input.portType === Enums.InternalPortType.MEOutput && input.id > 8000)
                    break;
                continue;
            default:
...
export function GetAudioInputsList(model, state, subset) {
    const getSource = (id, videoId, defLong) => {
    ...
    for (const input of model.classicAudio?.inputs ?? model.fairlightAudio?.inputs ?? []) {
        if (subset === 'delay' && (!('maxDelay' in input) || !input.maxDelay))
            continue;
        switch (input.portType) {
...
export function SourcesToChoices(sources) {
    return sources.map((s) => ({
        id: s.id,
        label: s.longName,
    }));
}
export function CameraControlSourcePicker() {
    return {
        id: 'cameraId',
        type: 'textinput',
        useVariables: true,
