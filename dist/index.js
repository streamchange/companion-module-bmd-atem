import AtemPkg, { Commands } from 'atem-connection';
import { GetActionsList } from './actions/index.js';
import { GetConfigFields } from './config.js';
import { GetFeedbacksList } from './feedback/index.js';
import { FeedbackId } from './feedback/FeedbackId.js';
import { GetAutoDetectModel, GetModelSpec, GetParsedModelSpec } from './models/index.js';
import { GetPresetsList } from './presets/index.js';
import { MODEL_AUTO_DETECT } from './models/types.js';
import { InitVariables, updateChangedVariables, updateDeviceIpVariable, } from './variables/lib.js';
 ...
 */
class AtemInstance extends InstanceBase {
    model;
    atem;
    wrappedState;
    commandBatching;
    ...
    updateCompanionBits() {
        ...
        this.setActionDefinitions(GetActionsList(this, this.atem, this.model, this.commandBatching, this.atemTransitions, this.wrappedState));
        this.checkFeedbacks();
    }
    ...
    processReceivedCommands(commands) {
        ...
        if (this.config.enableCameraControl && cameraCommands.length > 0) {
            ...
            const changedCameraIds = new Set();
     ...
     */
    processStateChange(newState, paths) {
        ...
        if (reInit) {
            ...
            if (changedFeedbacks.size > 0)
                this.checkFeedbacks(...Array.from(changedFeedbacks));
            if (changedFeedbackIds.size > 0)
                this.checkFeedbacksById(...Array.from(changedFeedbackIds));
        }
    }
    invalidateCachedTallyState() {
        // Invalidate any tally
        const changedFeedbackIds = new Set();
        ...
        this.atem.on('connected', () => {
            if (this.atem?.state) {
                ...
                if (newModelSpec) {
                    ...
                    this.updateStatus(InstanceStatus.UnknownWarning, `Unknown model: ${atemInfo.productIdentifier}. Some bits may be missing`);
                }
