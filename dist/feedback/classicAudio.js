...
export function createClassicAudioFeedbacks(model, state) {
    ...
    return {
        [FeedbackId.ClassicAudioGain]: {
            ...
            callback: ({ options }) => {
                ...
                return !!(channel && compareNumber(options.getPlainNumber('gain'), options.getPlainString('comparitor'), channel.gain));
            },
            learn: ({ options }) => {
                const audioChannels = state.state.audio?.channels ?? {};
                const channel = audioChannels[options.getPlainNumber('input')];
        ...
        [FeedbackId.ClassicAudioMasterGain]: {
            ...
            callback: ({ options }) => {
                ...
                return !!(props && compareNumber(options.getPlainNumber('gain'), options.getPlainString('comparitor'), props.gain));
            },
            learn: ({ options }) => {
                const props = state.state.audio?.master;
                if (props) {
