...
export function createFairlightAudioFeedbacks(model, state) {
    ...
    return {
        [FeedbackId.FairlightAudioInputGain]: {
            ...
            callback: ({ options }) => {
                ...
                return !!(source?.properties &&
                    compareNumber(options.getPlainNumber('gain'), options.getPlainString('comparitor'), source.properties.gain / 100));
            },
            learn: ({ options }) => {
                const audioChannels = state.state.fairlight?.inputs ?? {};
                const audioSources = audioChannels[options.getPlainNumber('input')]?.sources ?? {};
        ...
        [FeedbackId.FairlightAudioFaderGain]: {
            ...
            callback: ({ options }) => {
                ...
                const source = audioSources[options.getPlainString('source')];
                return !!(source?.properties &&
                    compareNumber(options.getPlainNumber('gain'), options.getPlainString('comparitor'), source.properties.faderGain / 100));
            },
            learn: ({ options }) => {
                const audioChannels = state.state.fairlight?.inputs ?? {};
                const audioSources = audioChannels[options.getPlainNumber('input')]?.sources ?? {};
        ...
        [FeedbackId.FairlightAudioMonitorMasterMuted]: model.fairlightAudio.monitor
            ...
            : undefined,
        [FeedbackId.FairlightAudioMonitorMasterGain]: model.fairlightAudio.monitor === 'split'
            ? {
                type: 'boolean',
        ...
        [FeedbackId.FairlightAudioMonitorTalkbackMuted]: model.fairlightAudio.monitor
            ...
            : undefined,
        [FeedbackId.FairlightAudioMonitorTalkbackGain]: model.fairlightAudio.monitor === 'split'
            ? {
                type: 'boolean',
            ...
            : undefined,
        [FeedbackId.FairlightAudioMonitorSidetoneGain]: model.fairlightAudio.monitor === 'split'
            ? {
                type: 'boolean',
