...
import { AtemSuperSourceIdPicker, AtemSuperSourceArtPropertiesPickers, AtemSuperSourceArtSourcePicker, AtemSuperSourceArtOption, AtemSuperSourceBoxPicker, AtemSuperSourceBoxSourcePicker, AtemSuperSourcePropertiesPickers, } from '../input.js';
import { getSuperSourceBox } from '../state.js';
function compareAsInt(value, actual, targetScale, actualRounding = 1) {
...
export function createSuperSourceFeedbacks(model, state) {
    if (!model.SSrc) {
        return {
    ...
    return {
        [FeedbackId.SSrcArtProperties]: {
            ...
            callback: ({ options }) => {
                ...
                if (!ssrc || !props || !Array.isArray(props))
                    return false;
                if (props.includes('fill') && ssrc.artFillSource !== options.getPlainNumber('fill'))
                    return false;
                if (props.includes('key') && ssrc.artCutSource !== options.getPlainNumber('key'))
                    return false;
                if (props.includes('artOption') && ssrc.artOption !== options.getRaw('artOption'))
                    return false;
        ...
        [FeedbackId.SSrcBoxProperties]: {
            ...
            callback: ({ options }) => {
                ...
                if (!box || !props || !Array.isArray(props))
                    return false;
                if (props.includes('source') && box.source !== options.getPlainNumber('source'))
                    return false;
                if (props.includes('size') && !compareAsInt(options.getPlainNumber('size'), box.size, 1000, 10))
                    return false;
                if (props.includes('x') && !compareAsInt(options.getPlainNumber('x'), box.x, 100))
                    return false;
                if (props.includes('y') && !compareAsInt(options.getPlainNumber('y'), box.y, 100))
                    return false;
                if (props.includes('cropEnable') && box.cropped !== options.getPlainBoolean('cropEnable'))
                    return false;
                if (box.cropped) {
